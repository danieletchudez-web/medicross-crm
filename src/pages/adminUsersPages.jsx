...
export default function AdminUsersPage({ profile, onNavigate }) {
  ...
  function setFullAccess(user) {
    updateUser(user.id, {
      approved: true,
      allowed_modules: MODULES.map((m) => m.id),
    });
  }
      ...
      <div className="admin-page">
        <section className="admin-hero">
          <div>
            ...
            <p>
              Aprobá accesos, definí roles y habilitá módulos visibles para cada usuario.
            </p>
          </div>
...
function UserRow({
  ...
  return (
    <tr>
      ...
      <td>
        <button
          className={`status-pill ${user.approved ? "approved" : "pending"}`}
          onClick={user.approved ? onBlock : onApprove}
          disabled={saving}
        >
