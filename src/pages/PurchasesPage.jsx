...
export default function PurchasesPage({ profile, onNavigate, navigationData, pageKey }) {
  ...
  const managerProfile = ["super_admin", "admin", "manager", "purchasing_manager", "team_lead"].includes(profile?.role) || ["administracion", "manager"].includes(profile?.department);
  const [filters, setFilters] = useState({ search: "", status: "active", priority: "", owner: managerProfile ? "all" : "mine_queue", deadline: "" });
  ...
  const visible = useMemo(() => rows.filter(row => {
    ...
    if (filters.owner === "mine_queue" && row.purchasing_owner_id && row.purchasing_owner_id !== profile?.id) return false;
  ...
  if (selectedId) {
    const selected = rows.find(row => row.quotation_id === selectedId);
    const readOnly = Boolean(!managerProfile && selected?.purchasing_owner_id !== profile?.id);
    ...
    <div className="p-page purchases-page">
      ...
      <QuotationWorkflow quotationId={selectedId} profile={profile} context="purchases" readOnly={readOnly} />
    </div>
    </Layout>;
  }
    ...
    <div className="p-page purchases-page">
      ...
      <section className="purchases-panel">
        <div className="purchases-filters">
          ...
          <select value={filters.status} onChange={e => setFilters({ ...filters, status: e.target.value })}><option value="active">Solicitudes activas</option><option value="pendiente_costos">Nuevas</option><option value="en_gestion_compras">En gestión</option><option value="costos_parciales">Parciales</option><option value="costos_completos">Completas</option></select>
          ...
          <select value={filters.owner} onChange={e => setFilters({ ...filters, owner: e.target.value })}><option value="mine_queue">Mi bandeja y sin asignar</option><option value="mine">Mis solicitudes</option><option value="unassigned">Sin asignar</option><option value="all">Todas (ajenas en solo lectura)</option></select>
