...
const COLS = [
  ...
  { key:"_actions",       label:"",                  w:70,  fixed:true },
];
...
export default function TendersPage({ profile, onNavigate }) {
  ...
  const filtered = useMemo(()=>{
    ...
    if (globalQ) {
      ...
      rows = rows.filter(t =>
        Object.values(t).some(v => v && String(v).toLowerCase().includes(q))
      );
    }
    // filtros por columna
  ...
  async function deleteTender(id,e){ e?.stopPropagation(); if(!confirm("¿Eliminar?")) return; await supabase.from("tenders").delete().eq("id",id); setTenders(p=>p.filter(t=>t.id!==id)); }
      ...
      <div className="tn-page">
        ...
        <div className="tn-header">
          ...
          <div className="tn-header__actions">
            {hasFilters && <button className="tn-btn tn-btn--ghost tn-btn--sm" onClick={()=>{setGlobalQ("");setColFilters({});}}>✕ Limpiar filtros</button>}
            ...
            <div className="tn-grid-scroll">
              <table className="tn-grid">
                ...
                <thead>
                  <tr className="tn-grid__head-row">
                    {COLS.map(col=>(
                      <th
                        ...
                        onClick={()=>col.key[0]!=="_"&&toggleSort(col.key)}
                      >
                  ...
                  {/* HEADER fila 2: filtros por columna */}
                  <tr className="tn-grid__filter-row">
                    {COLS.map(col=>(
                      <th key={col.key} className="tn-grid__filter-cell" style={{minWidth:col.w,maxWidth:col.w,width:col.w}}>
                        {col.key[0]!=="_" ? (
                          <input
                            className="tn-grid__filter-input"
                            placeholder="Filtrar…"
            ...
            <div className="tn-modal__body">
              ...
              <div className="tn-form-section">
                ...
                <div className="tn-form-grid">
                  ...
                  <div className="tn-field"><label>Hospital / Institución</label><input value={form.institution} onChange={e=>setF("institution",e.target.value)} placeholder="Nombre del hospital"/></div>
              ...
              <div className="tn-form-section">
                ...
                <div className="tn-form-grid tn-form-grid--1">
                  <div className="tn-field"><label>Nombre del proceso</label><input value={form.process_name} onChange={e=>setF("process_name",e.target.value)} placeholder="Descripción completa"/></div>
                </div>
                <div className="tn-form-grid">
