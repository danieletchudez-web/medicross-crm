...
function InlineAttachments({ tenderId }) {
  ...
  return (
    <div className="tn-inline-attach">
      ...
      : files.length === 0 ? <p className="tn-attach-empty">Sin archivos adjuntos todavía.</p>
      : (
...
export default function TendersPage({ profile, onNavigate }) {
  ...
  const kpis = useMemo(() => {
    ...
    const adjMontos  = tenders.filter(t => ["Adjudicada","Orden de compra recibida","En ejecución","Entrega parcial","Entregada","Facturada","Cobrada"].includes(t.operational_status))
    ...
    const ganadas    = tenders.filter(t => ["Adjudicada","Orden de compra recibida","En ejecución","Entrega parcial","Entregada","Facturada","Cobrada"].includes(t.operational_status)).length;
    return { activas:activas.length, montoTotal, adjMontos, proxVencer, sinAccion, ganadas, total:tenders.length };
  }, [tenders]);
  ...
  function dismissAlert(key) { setDismissedAlerts(prev => new Set([...prev, key])); }

  function openNew() { setEditData(null); setForm({...EMPTY_FORM, detection_date: today()}); setShowForm(true); }
  ...
  function exportToExcel() {
    ...
    const headers = ["Jurisdicción","Hospital/Institución","N° Proceso","Nombre Proceso","Expediente","Tipo Proceso","Tipo","Detección","Vencimiento","Estado Operativo","Prioridad","Monto OC","Responsable","Línea Producto","Próxima Acción","Fecha Próx. Acción","Documentación","Facturación","Entrega","N° OC","Fecha OC","Inicio","Póliza","OT Bridge","Plazo","Sector","Portal","Observaciones"];
    const keys    = ["jurisdiction","institution","process_number","process_name","expedient_number","process_type","tender_type","detection_date","end_date","operational_status","priority","purchase_order_amount","internal_owner","product_line","next_action","next_action_date","documentation_status","billing_status","delivery_status","purchase_order_number","purchase_order_date","start_date","execution_policy","bridge_ot","contract_term","requesting_sector","portal_link","notes"];
  ...
  function renderCell(col, t) {
    ...
    switch(col.key) {
      ...
      case "notes":
        return <span style={{fontSize:11,color:"#64748b",display:"block",maxWidth:200,overflow:"hidden",
          textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={t.notes||""}>{t.notes||"—"}</span>;
      default:
      ...
      <div className="tn-page">
        ...
        <div className="tn-kpis">
          ...
          <div className="tn-kpi tn-kpi--green">
            ...
            <span className="tn-kpi__sub">{kpis.ganadas} con OC</span>
          </div>
          <div className="tn-kpi tn-kpi--gray">
            ...
            <span className="tn-kpi__sub">en el pipeline</span>
          </div>
        </div>
        ...
        <div className="tn-search-bar">
          <input className="tn-search-input"
            placeholder="🔍  Buscar hospital, proceso, expediente, sector…"
                ...
                <tbody>
                  {filtered.length===0 ? (
                    ...
                    <tr key={t.id} className={`tn-grid__row ${idx%2===0?"":"tn-grid__row--alt"}`}
                      style={{cursor:"pointer"}} onClick={()=>openEdit(t)}>
                      {COLS.map(col => (
                        <td key={col.key} className={`tn-grid__td ${col.fixed?"tn-grid__td--fixed":""}`}
                          style={{minWidth:col.w,maxWidth:col.w,width:col.w}}>
                          {renderCell(col,t)}
                        </td>
                      ))}
        ...
        <div className="tn-overlay" onClick={e=>{if(e.target.classList.contains("tn-overlay"))setShowForm(false);}}>
          <div className="tn-modal" style={{maxWidth:860}}>
            ...
            <div className="tn-modal__body">
              ...
              <div className="tn-form-section">
                <p className="tn-form-section__title">💰 Estimación económica</p>
                <div className="tn-form-grid tn-form-grid--3">
              ...
              {/* § 5 OC (si está adjudicada) */}
              <div className="tn-form-section">
                <p className="tn-form-section__title">🧾 Orden de compra (si adjudicada)</p>
                <div className="tn-form-grid tn-form-grid--3">
              ...
              <div className="tn-form-section">
                ...
                {editData ? <InlineAttachments tenderId={editData.id}/> : (
                  <div className="tn-inline-attach">
