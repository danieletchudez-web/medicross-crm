...
function CostEditor({ item, profile, onSaved, readOnly = false }) {
  ...
  return (
    <article className={`qwf-item ${item.cost_available ? "qwf-item--validated" : ""} ${readOnly ? "qwf-item--readonly" : ""}`}>
      ...
      <div className="qwf-form-grid">
        ...
        <label className="qwf-cost-primary">Costo unitario<input type="number" min="0" step="0.01" placeholder="Ingresar costo" value={form.unit_cost} onChange={e => change("unit_cost", e.target.value)} disabled={item.cost_available}/><small>Al ingresar un importe cambia a “Costo cargado”.</small></label>
          ...
          <label>IVA %<select value={String(form.vat_pct)} onChange={e => change("vat_pct", Number(e.target.value))} disabled={item.cost_available}><option value="21">21%</option><option value="10.5">10,5%</option></select></label>
      ...
      <footer>{base.version && <small>Versión de costo {base.version} · {money(base.total_unit_cost || base.unit_cost, base.currency)}</small>}{!readOnly && <button type="button" onClick={save} disabled={saving || item.cost_available}>{saving ? "Guardando…" : resolved ? "Guardar versión de costo" : "Guardar resolución pendiente"}</button>}</footer>
    </article>
  );
}
...
export default function QuotationWorkflow({ quotationId, profile, context = "cotizador", readOnly = false }) {
  ...
  const effectiveReadOnly = readOnly && data.quote.purchasing_owner_id !== profile?.id;
  ...
  const importPurchasesExcel = async event => {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const values = { ...row, status, currency:row.currency || "ARS", vat_pct:Number(row.vat_pct) === 10.5 ? 10.5 : 21, confidence:row.confidence || "pendiente_confirmacion" };
  ...
  return <section className="qwf-shell">
    ...
    {context === "purchases" && effectiveReadOnly && <div className="qwf-readonly"><b>Vista de solo lectura</b><span>{data.quote.purchasing_owner_id ? "Esta gestión está asignada a otro comprador." : "Tomá la gestión para cargar o validar costos."}</span></div>}
    ...
    <div className="qwf-body">
      ...
      {tab === "costs" && <><div className="qwf-section-head"><div><h4>Gestión de costos</h4><p>Los avances se guardan por renglón y cada costo genera una versión.</p></div><div className="qwf-cost-actions"><button className="qwf-secondary" onClick={exportPurchasesPdf}>PDF</button><button className="qwf-secondary" onClick={exportPurchasesExcel}>Excel</button>{!effectiveReadOnly && <><button className="qwf-secondary" onClick={() => importInputRef.current?.click()}>Importar Excel</button><input ref={importInputRef} type="file" accept=".xlsx,.xls" hidden onChange={importPurchasesExcel}/></>}{isPurchasing(profile) && !effectiveReadOnly && <button onClick={() => setValidationOpen(true)}>Validar cotización</button>}</div></div>{data.items.map(item => <CostEditor key={item.id} item={item} profile={profile} readOnly={effectiveReadOnly} onSaved={load}/>)}</>}
