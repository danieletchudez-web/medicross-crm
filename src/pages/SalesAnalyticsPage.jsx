  ...
  return (
    <Layout title="Análisis Comercial" profile={profile} onNavigate={onNavigate}>
      <div className="p-page">
        ...
        {/* KPIs + PERIOD FILTER PANEL */}
        <div className="p-panel">
          <div className="p-hd">
            <div className="p-hd-left">
              <span className="p-title">Comparativa de vendedores</span>
              <span className="p-sub">Análisis de actividad, pipeline y productividad del equipo</span>
            </div>
            <div className="p-hd-right">
              <div className="p-pills">
                {PERIOD_OPTIONS.map((p) => (
                  <button key={p.key} className={`p-pill ${period === p.key ? "p-pill--active" : ""}`} onClick={() => setPeriod(p.key)}>{p.label}</button>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="p-metrics">
            <div className="p-metric">
              <span className="p-metric__ey">Visitas del equipo</span>
              <span className="p-metric__val">{totals.visits}</span>
              <span className="p-metric__sub">en el período</span>
            </div>
            <div className="p-metric">
              <span className="p-metric__ey">Opps. creadas</span>
              <span className="p-metric__val">{totals.opps}</span>
              <span className="p-metric__sub">oportunidades</span>
            </div>
            <div className="p-metric">
              <span className="p-metric__ey">Pipeline total</span>
              <span className="p-metric__val">{compactMoney(totals.pipeline)}</span>
              <span className="p-metric__sub">oportunidades abiertas</span>
            </div>
            <div className="p-metric">
              <span className="p-metric__ey">Opp. ganadas</span>
              <span className="p-metric__val p-metric__up">{totals.won}</span>
              <span className="p-metric__sub">en el período</span>
            </div>
            <div className="p-metric">
              <span className="p-metric__ey">Conversión promedio</span>
              <span className={`p-metric__val ${totals.convRate >= 40 ? "p-metric__up" : totals.convRate >= 20 ? "" : "p-metric__down"}`}>{totals.convRate}%</span>
              <span className="p-metric__sub">tasa de cierre</span>
            </div>
            {best && (
              <div className="p-metric">
                <span className="p-metric__ey">Top vendedor</span>
                <span className="p-metric__val" style={{ color: "#f59e0b" }}>{best.name.split(" ")[0]}</span>
                <span className="p-metric__sub">Score {best.score}/100</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-panel">
          <div className="p-hd">
            <div className="p-hd-left">
              <span className="p-title">Vendedores</span>
            </div>
            <div className="p-hd-right">
              <button className="p-btn p-btn--ghost" onClick={selectAll}>Todos</button>
              <button className="p-btn p-btn--ghost" onClick={selectNone}>Ninguno</button>
            </div>
          </div>
          <div className="p-body">
            <div className="p-pills">
              {sellerMetrics.map((s) => (
                <button
                  key={s.id}
                  className={`p-pill ${selected.includes(s.id) ? "p-pill--active" : ""}`}
                  style={selected.includes(s.id) ? { borderColor: s.color, background: s.color + "15", color: s.color } : {}}
                  onClick={() => toggleSeller(s.id)}
                >
                  <span className="p-dot--gray" style={{ background: s.color, display: "inline-block", width: 8, height: 8, borderRadius: "50%", marginRight: 6 }}/>
                  {s.name.split(" ")[0]}
                </button>
              ))}
            </div>
        ...
        <div className="p-panel">
          <div className="p-toolbar--top">
            <div className="p-tabs">
              {[
                ...
                <button key={t.key} className={`p-tab ${activeTab === t.key ? "p-tab--active" : ""}`} onClick={() => setActiveTab(t.key)}>
                  {t.label}
                  {t.key === "alerts" && totals.alerts > 0 && (
                    <span className="p-badge--red" style={{ marginLeft: 6 }}>{totals.alerts}</span>
                  )}
                </button>
              ))}
            </div>
          ...
          {activeTab === "graficos" && (
            <div className="p-body">
              <div className="sa-charts-grid">
                <div className="p-panel p-panel--span2">
                  <div className="p-hd">
                    <div className="p-hd-left">
                      <span className="p-title">Actividad comparativa</span>
                      <span className="p-sub">Visitas y oportunidades ganadas por vendedor</span>
                    </div>
                  </div>
                  <div className="p-chart"><canvas ref={barRef}/></div>
                </div>
                <div className="p-panel">
                  <div className="p-hd">
                    <div className="p-hd-left">
                      <span className="p-title">Radar multidimensional</span>
                      <span className="p-sub">Perfil comercial comparado</span>
                    </div>
                  </div>
                  <div className="p-chart"><canvas ref={radarRef}/></div>
                </div>
                <div className="p-panel p-panel--span2">
                  <div className="p-hd">
                    <div className="p-hd-left">
                      <span className="p-title">Pipeline por vendedor</span>
                      <span className="p-sub">Valor de oportunidades abiertas</span>
                    </div>
                  </div>
                  <div className="p-chart"><canvas ref={lineRef}/></div>
                </div>
              </div>

              <div className="p-panel" style={{ marginTop: 14 }}>
                <div className="p-hd">
                  <div className="p-hd-left">
                    <span className="p-title">Actividad por día de semana</span>
          ...
          {activeTab === "ranking" && (
            <div className="p-body">
              <div className="p-list">
                {ranked.map((s, i) => (
                  <div key={s.id} className="p-row" style={{ padding: "14px 20px", alignItems: "center", gap: 14 }}>
                    <span className="p-row__rank" style={{ color: i === 0 ? "#f59e0b" : i === 1 ? "#94a3b8" : i === 2 ? "#cd7c3a" : "#64748b", fontSize: 18 }}>
                      {i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `#${i+1}`}
                    </span>
                    <div className="p-avatar" style={{ background: s.color, flexShrink: 0 }}>
                      {s.name.slice(0,1).toUpperCase()}
                    </div>
                    <div className="p-row__main">
                      <span className="p-row__name">{s.name}</span>
                      <span className="p-row__sub">{s.email}</span>
                    </div>
                    <div className="p-progress" style={{ flex: 1, maxWidth: 180 }}>
                      <div className="p-progress-fill" style={{ width: `${s.score}%`, background: s.color }}/>
                    </div>
                    <span className="p-row__val" style={{ color: s.color, minWidth: 48 }}>{s.score}<small style={{ color: "#64748b" }}>/100</small></span>
                    <div className="p-row__meta" style={{ display: "flex", gap: 16 }}>
                ...
                {ranked.length === 0 && <p className="p-empty">Seleccioná al menos un vendedor.</p>}
              </div>
            </div>
          )}
                ...
                <tbody>
                  {filtered.map((s) => (
                    <tr key={s.id}>
                      ...
                      <td>
                        <span className={`p-badge--${s.convRate >= 40 ? "green" : s.convRate >= 20 ? "amber" : "red"}`}>{s.convRate}%</span>
                      </td>
          ...
          {activeTab === "alerts" && (
            <div className="p-body">
              {/* Team health summary */}
              {filtered.length > 0 && (
                <div className="p-metrics" style={{ marginBottom: 18 }}>
                  <div className="p-metric">
                    <span className="p-metric__ey">Sin actividad</span>
                    <span className="p-metric__val">{filtered.filter((s) => s.totalVisits === 0).length}</span>
                  </div>
                  <div className="p-metric">
                    <span className="p-metric__ey">Opps. vencidas</span>
                    <span className={`p-metric__val ${filtered.reduce((s, x) => s + x.overdueOpps, 0) > 0 ? "p-metric__down" : ""}`}>{filtered.reduce((s, x) => s + x.overdueOpps, 0)}</span>
                  </div>
                  <div className="p-metric">
                    <span className="p-metric__ey">Sin próx. acción</span>
                    <span className={`p-metric__val ${filtered.reduce((s, x) => s + x.noNextAction, 0) > 0 ? "p-metric__down" : ""}`}>{filtered.reduce((s, x) => s + x.noNextAction, 0)}</span>
                  </div>
                  <div className="p-metric">
                    <span className="p-metric__ey">Clientes en riesgo</span>
                    <span className={`p-metric__val ${filtered.reduce((s, x) => s + x.redAccounts, 0) > 0 ? "p-metric__down" : ""}`}>{filtered.reduce((s, x) => s + x.redAccounts, 0)}</span>
                  </div>
                </div>
              )}
              ...
              <div className="p-list">
                ...
                {filtered.length === 0 && <p className="p-empty">Seleccioná al menos un vendedor.</p>}
              </div>
            </div>
          )}
        </div>
